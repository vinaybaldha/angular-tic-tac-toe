    ...
    <div class="container">
      <div class="leftArea">
        <div id="statusArea" className="status">
          Next player:
          <span class="color-{{ currentPlayer }}">{{ currentPlayer }}</span>
        </div>
        <div id="winnerArea" className="winner">
          Winner:
          <span class="winner-name color-{{ winnerPlayer }}">{{
            winnerPlayer
          }}</span>
        </div>
      </div>
      <section class="gridArea">
        <div class="row" *ngFor="let row of matrix; let i = index">
          <button
            *ngFor="let symbol of matrix[i]; let j = index"
            class="square"
            (click)="select(i, j)"
          >
            <ng-container *ngIf="symbol === 1"
              ><span class="color-x">X</span></ng-container
            >
            <ng-container *ngIf="symbol === 2"
              ><span class="color-o">O</span></ng-container
            >
          </button>
        </div>
        <div
          class="winner-overlay color-{{ winnerPlayer }}"
          *ngIf="winnerPlayer !== 'none'"
        >
          {{ winnerPlayer }} wins!!
        </div>
      </section>
